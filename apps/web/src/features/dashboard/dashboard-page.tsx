...
export function DashboardPage() {
  ...
  const loading =
    dashboard.isLoading || meetings.isLoading || notifications.isLoading
  const error = dashboard.isError || meetings.isError || notifications.isError
  if (loading) return <DashboardSkeleton />
  if (error || !dashboard.data || !meetings.data || !notifications.data)
    return (
      <div
      ...
      <section className="grid gap-5 lg:grid-cols-[1.55fr_1fr]">
        <article className="overflow-hidden rounded-2xl border bg-card shadow-sm">
          <header className="flex items-center justify-between border-b p-5">
            ...
            <Link className="text-sm font-semibold text-primary" to="/calendar">
              Open calendar
            </Link>
          </header>
          {scheduled.length ? (
